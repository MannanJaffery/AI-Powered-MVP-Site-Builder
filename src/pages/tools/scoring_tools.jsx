...
useEffect(() => {
  const interval = setInterval(() => {
    if (isLimited) {
      const usageData = JSON.parse(localStorage.getItem("usageHistory") || "{}");
      const history = usageData["IdeaRanker"] || [];
          ...
          <ul className="hidden md:flex space-x-8 text-sm font-medium">
            ...
            <li className="relative">
              ...
              >
                ...
                <ChevronDown
                  size={16}
                  className={`ml-1 transition-transform duration-300 ${openTools ? "rotate-180 text-green-600" : ""
                    }`}
                />
              </button>
...
<div className="flex items-center gap-3">
  ...
  <div className="hidden sm:flex items-center gap-2 bg-green-50 px-3 py-1.5 rounded-full border border-green-200">
    <span className="text-xs font-medium text-green-800">
      {limit - usageCount} / {limit} uses remaining
    </span>
  </div>

  {/* Mobile Usage Counter */}
  <div className="flex sm:hidden items-center gap-2 bg-green-50 px-3 py-1.5 rounded-full border border-green-200">
    <span className="text-xs font-medium text-green-800">
      {limit - usageCount} / {limit} uses
    </span>
  </div>

  {/* Beta Badge */}
  {/* Only visible on medium screens and up */}
  <span className="hidden sm:block text-xs font-medium bg-green-100 text-green-800 px-2.5 py-1 rounded-full transition-all hover:bg-purple-200">
    BETA
  </span>
</div>
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 mb-8" style={{ borderTop: '6px solid #46AA72' }}>
          ...
          >
            {loading ? (
              ...
              <>
                <span>Try again in {timeDisplay}</span>
              </>
            ) : (
              <>
